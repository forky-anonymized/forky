...
namespace {
const TestingSetup* g_setup = nullptr;
} // namespace
...
std::array<std::optional<node::BlockfileCursor>, node::BlockfileType::NUM_TYPES> rev_m_blockfile_cursors;
...
// CCoinsViewBacked rev_backed_base;
std::unique_ptr<CCoinsViewCache> rev_cvcache;
uint256 rev_hashBlock;
size_t rev_cachedCoinsUsage;

typedef struct ReorgType {
    int n_replacing_blocks;
    int n_replaced_blocks;
    int nth_reorg;
}ReorgType;

auto ReorgTypeCmp = [](const ReorgType a, const ReorgType b) {
    auto res = a.nth_reorg < b.nth_reorg;
    if (a.nth_reorg == b.nth_reorg) res = a.n_replacing_blocks < b.n_replacing_blocks;
    if (a.nth_reorg == b.nth_reorg && a.n_replacing_blocks == b.n_replacing_blocks) res = a.n_replaced_blocks < b.n_replaced_blocks;
    return res;
};

int replacing_blocks;
int replaced_blocks;
int nth_reorg;
std::set<ReorgType, decltype(ReorgTypeCmp)> reorgs(ReorgTypeCmp);
...
bool rev_m_load_tried{false};
CFeeRate rev_m_min_relay_feerate;
indexed_transaction_set rev_mapTx;
using txiter = indexed_transaction_set::nth_index<0>::type::const_iterator;
std::vector<std::pair<uint256, txiter>> rev_vTxHashes;
std::vector<CTransactionRef> rev_txns_randomized;
...
CBlockIndex* rev_m_best_header = nullptr;
uint256 rev_g_best_block;
auto rev_nMaxTipAge = DEFAULT_MAX_TIP_AGE;
...
void SaveBlockChain()
{
    ...
    for (auto& index : blockman.m_block_index) {
        ...
        rev_BlockIndexes.emplace(index.first, CBlockIndex(index.second));
    }
    ...
    // v23 -> v24
    rev_m_best_header = csm->m_best_header;

    rev_m_chain = cs.m_chain.vChain;

    // v25 -> v26
    // Theses three moved from cs to csm
    rev_nBlockSequenceId = csm->nBlockSequenceId;
    rev_nBlockReverseSequenceId = csm->nBlockReverseSequenceId;
    rev_nLastPreciousChainwork = csm->nLastPreciousChainwork;

    rev_m_cached_finished_ibd = csm->m_cached_finished_ibd.load();
    ...
    rev_m_blockfile_cursors = bm.m_blockfile_cursors;
    ...
    auto& cv = cs.m_coins_views->m_cacheview;
    auto& cv_fuzz = cs.m_coins_views->m_cacheview_fuzz;
    
    // v25 -> v26
    // rev_m_cacheview = std::make_shared<CCoinsViewCache>(CCoinsViewCache(&rev_backed_base));
    // rev_cvcache = std::make_unique<CCoinsViewCache>(CCoinsViewCache(cv->base));
    ...
    rev_cachedCoinsUsage = cv->cachedCoinsUsage;
    rev_hashBlock = cv->hashBlock;
    for (auto& entry : cv->cacheCoins) {
        cv_fuzz->cacheCoins.emplace(std::piecewise_construct, std::forward_as_tuple(entry.first), std::forward_as_tuple(entry.second));
        // rev_cvcache->cacheCoins.emplace(std::piecewise_construct, std::forward_as_tuple(entry.first), std::forward_as_tuple(entry.second));
    }
    ...
    auto& mp = cs.m_mempool;
    {
        ...
        rev_m_load_tried = mp->m_load_tried;
        rev_m_min_relay_feerate = mp->m_min_relay_feerate;

        // rev_mapTx = mp->mapTx;
        // rev_vTxHashes = mp->vTxHashes;
        // v26 -> v27
        rev_txns_randomized = mp->txns_randomized;
...
void initialize_block()
{
    ...
    // static const auto testing_setup = std::make_unique<const TestingSetup>(ChainType::REGTEST);
    ...
    // v25 -> v26
    // CAutoFile cfile{file, CLIENT_VERSION};
    // v26 -> v27
    AutoFile cfile{file};
    ...
    g_setup->m_node.chainman->LoadExternalBlockFile(cfile);
    for (Chainstate* chainstate : WITH_LOCK(::cs_main, return g_setup->m_node.chainman->GetAll())) {
...
FUZZ_TARGET(block, .init = initialize_block)
{
    ...
    for (auto& block : TestCase)
    {
        ...
        replacing_blocks = 0;
        BlockValidationState state;
        auto key = "Block " + padStringWithZeros(std::to_string(nBlock), 2);
        bool fFirstSeen{false}, fAccepted{false};
        // !!!!!!! FuzzProcessNewBlock has a side effect of updating replaced_blocks and replacing_blocks
        bool fProcessed = g_setup->m_node.chainman
                        ->FuzzProcessNewBlock (state, std::make_shared<CBlock>(block.block), true, true, &fFirstSeen);
        // std::cout << "\t\tFuzzProcessNewBlock: " << std::boolalpha << fProcessed << (fFirstSeen ? ", new block" : ", not new block") << std::endl;
        {
            ...
            // Reorg Feedback
            std::cout << "ReorgType: " << nth_reorg << ", replaced_blocks: " << replaced_blocks << ", replacing_blocks: " << replacing_blocks << std::endl;
            if (replaced_blocks != 0 && replacing_blocks != 0) {
                nth_reorg++;
                auto new_type = ReorgType{replaced_blocks, replacing_blocks, nth_reorg};
                auto is_unique_reorg = (reorgs.find(new_type) == reorgs.end());
                has_unique_reorg |= is_unique_reorg;
                if (is_unique_reorg) reorgs.insert(new_type);
            }
        }
    ...
    auto& activeChainStateManager = g_setup->m_node.chainman;
    ...
    WITH_LOCK(cs_main, activeChainStateManager->nBlockSequenceId = rev_nBlockSequenceId);
    activeChainStateManager->nBlockReverseSequenceId = rev_nBlockReverseSequenceId;
    activeChainStateManager->nLastPreciousChainwork = rev_nLastPreciousChainwork;
    activeChainStateManager->m_cached_finished_ibd = rev_m_cached_finished_ibd.load();
    ...
    // v23 -> v24
    g_setup->m_node.chainman->m_best_header = rev_m_best_header;
    ...
    blockman.m_blockfile_cursors = rev_m_blockfile_cursors;
    ...
    // step 5 : revert UTXO cache
    {
        ...
        // v25 -> v26
        cv->hashBlock = rev_hashBlock;
        cv->cachedCoinsUsage = rev_cachedCoinsUsage;
        ...
        cv->cacheCoins.clear();
        for (auto& entry : g_setup->m_node.chainman->ActiveChainstate().m_coins_views->m_cacheview_fuzz->cacheCoins)
            cv->cacheCoins.emplace(std::piecewise_construct, std::forward_as_tuple(entry.first), std::forward_as_tuple(entry.second));
        // {
    ...
    auto& mp = activeChainState.m_mempool;
    {
        ...
        // v23 -> v24
        mp->m_load_tried = rev_m_load_tried;
        // mp->m_is_loaded = rev_m_is_loaded;
        // mp->m_min_relay_feerate = rev_m_min_relay_feerate;

        // mp->mapTx = rev_mapTx;
        // v26 -> v27
        // mp->vTxHashes = rev_vTxHashes;
        mp->txns_randomized = rev_txns_randomized;
    ...
    // If it is, overwrite the old block index (for disk file position)
    {
        ...
        for (auto& key : rev_keys)
            // blockman.m_block_index.insert(*rev_BlockIndexes.find(key));
            blockman.m_block_index[key] = rev_BlockIndexes[key];
        // v23 -> v24
        // Empty the blockman.m_block_index
        // blockman.m_block_index.clear();
        // // Insert the rev_BlockIndexes
        // for (auto& entry : rev_BlockIndexes) {
        //     blockman.m_block_index.insert(entry);
        // }
    }
    ...
    if (has_unique_reorg)
    {
        std::cout << "Current ReorgType count: " << reorgs.size() << std::endl;
        AddToCorpus(buffer.data(), buffer.size());
    }
    // else
    // {
    //     std::cout << "No new ReorgType, delete: " << HexStr(hashTestCase) << std::endl;
    //     auto path_testcase = fs::current_path() / "fuzzing_corpus" / PACKAGE_VERSION / HexStr(hashTestCase);
    //     fs::remove(path_testcase);
    // }
}
