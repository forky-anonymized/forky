...
public:
    ...
    // Forky: for fuzzing!
    std::unique_ptr<CCoinsViewCache> m_cacheview_fuzz GUARDED_BY(cs_main);
...
public:
    ...
    bool FuzzProcessNewBlock(BlockValidationState& state, const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block) LOCKS_EXCLUDED(cs_main);
