...
func (bb *Builder) Tick(t testing.TB, tick int64) {
	prysmTime.SetCurrentTime(bb.service.GenesisTime().Unix() + tick)
	...
	for lastSlot < currentSlot {
		lastSlot++
		bb.service.SetForkChoiceGenesisTime(uint64(prysmTime.Now().Unix() - int64(params.BeaconConfig().SecondsPerSlot*lastSlot)))
		require.NoError(t, bb.service.NewSlot(context.TODO(), primitives.Slot(lastSlot)))
		bb.service.UpdateHead(context.TODO(), bb.service.CurrentSlot())
	}
	if tick > int64(params.BeaconConfig().SecondsPerSlot*lastSlot) {
		bb.service.SetForkChoiceGenesisTime(uint64(prysmTime.Now().Unix() - tick))
	}
	bb.lastTick = tick
}
...
func (bb *Builder) Attestation(t testing.TB, a *ethpb.Attestation) {
	require.NoError(t, bb.service.SaveAttestation(a))
	bb.service.UpdateHead(context.TODO(), bb.service.CurrentSlot())
